...
BatchSize = 10 # (ミニバッチ数)
trainPath = r"datasets/train/addBG_r" # 学習用
valPath = r"datasets/val" # 推論テスト用
modelPath = r"result/model{}.pth" # モデルを保存するパス
onnxPath = r"result/model.onnx"
...
class Trainer():
	...
	def TrainEpoch(self, epoch):
		...
		for batchIdx, (data, target) in enumerate(self.trainLoader):
			...
			trainStr = "epoch : {:3} train_loss : {:3.5} train_acc : {:3.5}".format(str(epoch+1), str(totalTrainLoss/((batchIdx+1)*BatchSize)), str(100*trainCorrect.data.item() / ((batchIdx+1)*BatchSize)))
		...
		valStr = ""
		with torch.no_grad():
			for batchIdx, (data, target, imgPath) in enumerate(self.valLoader):
